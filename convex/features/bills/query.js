        ...
        for (const bill of bills) {
            ...
            for (const item of items) {
                ...
                if (unsettled.length === 0)
                    continue;
                const shareAmount = (item.unitPrice * item.quantity) / unsettled.length;
                for (const assignment of unsettled) {
                    if (assignment.userId === bill.payerId)
                        continue;
                    // Debtor owes money (negative)
                    balanceMap.set(assignment.userId, (balanceMap.get(assignment.userId) || 0) - shareAmount);
                    // Creditor is owed (positive)
                    balanceMap.set(bill.payerId, (balanceMap.get(bill.payerId) || 0) + shareAmount);
                }
            }
        }
        return users.map((u) => ({
            ...
            for (const bill of bills) {
                ...
                for (const item of items) {
                    ...
                    for (const assignment of unsettled) {
                        if (assignment.userId === bill.payerId)
                            continue;
                        balanceMap.set(assignment.userId, (balanceMap.get(assignment.userId) || 0) -
                            shareAmount);
                        balanceMap.set(bill.payerId, (balanceMap.get(bill.payerId) || 0) + shareAmount);
                    }
                }
