...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        let totalAmount = 0;
        const allAssignedUsers = new Set<Id<"users">>();

        for (const item of args.items) {
            if (item.unitPrice < 0) {
                throw new Error(
                    `Item "${item.name}" cannot have negative price`
                );
            }
        ...
        for (const item of args.items) {
            ...
            const shareAmount =
                (item.unitPrice * item.quantity) / item.assignedUserIds.length;
...
export const update = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        let totalAmount = 0;
        const allAssignedUsers = new Set<Id<"users">>([args.payerId]);
